...
/**
 * Filter Variants on the basis of the predicted pathogenicity. This class filters
 * both on variant class (NONSENSE, MISSENSE, INTRONIC) etc., as well as on the
 * basis of MutationTaster/Polyphen2/SIFT scores for mutations. 
...
public class PathogenicityTriage implements ITriage  {
    ...
    /** The overall value of the estimated pathogenicity of this mutation.
     * Must be a value between 0 and 1.
     * 0: Completely sure it is nonpathogenic
     * 1.0: Completely sure it is pathogenic
     */
    ...
    /**
     * Assumed pathogenicity score for a variant that causes the
     * start codon to be lost.*/
    ...
    */
     ...
     * category, thus, we are not being extremely strict with the polyphen filter).
     */
     ...
     * variants for which we cannot find values for mutationTaster, polyphen2, or SIFT.
     */
    ...
    /**
     * TODO CHeck whether this threshold is correct. Note it is the dbNSFP normalized version of mutation taster? */
    private static final float MTASTER_THRESHOLD = 0.94f;

    private static String missense_filtering = "no";

    /** This constructor is intended to be used for MISSENSE mutations, and the pathogenicity filter is
     * based upon the classification of the variant by polyphen, mutation taster and sift.
     * The constructor also calculates an overall pathogenicity score based on the predictions of 
     * the three pathogenicity programs.
     * @param poly Polyphen2 score
     * @param mt MutationTaster score
     * @param SIFT SIFT score
     */
    public PathogenicityTriage(float poly, float mt, float SIFT) {
	this.polyphen = poly;
     ...
     */
    public PathogenicityTriage(VariantType type,float pathogenicity) {
	...
	if (this.variantType == VariantType.MISSENSE) {
		if (sift_is_initialized() && this.sift < SIFT_THRESHOLD) return true;
	    if (polyphen_is_initialized() && this.polyphen > POLYPHEN_THRESHOLD ) return true;
	    if (mutation_taster_is_initialized() && this.mutation_taster > MTASTER_THRESHOLD) return true;
	    if (no_prediction_initialized()) return true;
    ...
    /**
     * This method creates a PathogenicityTriage object for certain kinds of variants.
     * Note that NONSYNONYMOUS variants are not evaluated in this way, rather they are
     * evaluated with specific scores (MutationTaster, SIFT, and Polyphen2). This function
     * basically just assigns a score based on the class of the variant using the 
     * function <i>getVariantTypeConstant</i>. Probably this can be simplified by
     * just using this directly. The only thing that this function does that is additional is
     * basically to check that the variant type is one of the registered types. TODO
     */
     public static PathogenicityTriage  evaluateVariantClass(Variant v) {
	VariantType cls = v.getVariantTypeConstant();
	if (! cls.isTopPriorityVariant()) {
	    return createNonPathogenicTriageObject(cls);
	}
	switch (cls) {

	case FS_DELETION: return createPathogenicTriageObjectByType(VariantType.FS_DELETION,FRAMESHIFT_SCORE);
	case FS_INSERTION: return createPathogenicTriageObjectByType(VariantType.FS_INSERTION,FRAMESHIFT_SCORE);
	case NON_FS_SUBSTITUTION: return createPathogenicTriageObjectByType(VariantType.NON_FS_SUBSTITUTION,NONFRAMESHIFT_INDEL_SCORE);
	case FS_SUBSTITUTION: return createPathogenicTriageObjectByType(VariantType.FS_SUBSTITUTION,FRAMESHIFT_SCORE);
	case NON_FS_DELETION: return createPathogenicTriageObjectByType(VariantType.NON_FS_DELETION,NONFRAMESHIFT_INDEL_SCORE);
	case NON_FS_INSERTION: return createPathogenicTriageObjectByType(VariantType.NON_FS_INSERTION,NONFRAMESHIFT_INDEL_SCORE);
	case SPLICING: return createPathogenicTriageObjectByType(VariantType.SPLICING,SPLICING_SCORE);
	case STOPGAIN: return createPathogenicTriageObjectByType(VariantType.STOPGAIN,NONSENSE_SCORE);
	case STOPLOSS: return createPathogenicTriageObjectByType(VariantType.STOPLOSS,STOPLOSS_SCORE); 
	    /* Note, the frameshift duplication get the FRAMESHIFT default score */
	case FS_DUPLICATION: return createPathogenicTriageObjectByType(VariantType.FS_DUPLICATION,FRAMESHIFT_SCORE);
	case NON_FS_DUPLICATION: return createPathogenicTriageObjectByType(VariantType.NON_FS_DUPLICATION,NONFRAMESHIFT_INDEL_SCORE);
	case START_LOSS: return createPathogenicTriageObjectByType(VariantType.START_LOSS, STARTLOSS_SCORE);
	}
     ...
     * Note that 0 means predicted to be non-pathogenic, and 1.0 means maximally
	...
	if (this.variantType != VariantType.MISSENSE) {
	    ...
	    int c=0; /* how many precitions do we have? */
	    String s=null;
    ...
    /**
     * This function returns a list with a summary of the pathogenicity analysis. It first
     * enters the variant type (see {@link jannovar.common.VariantType VariantType}), and then either
     * shopws the overall pathogenicity score (defined in several constants in this class such as
     {@link #FRAMESHIFT_SCORE}), or additionally shows the results of analysis by polyphen2,
     * MutationTaster, and SIFT. This list can be displayed as an HTML list if desired (see
  ...
  @Override public String getHTMLCode() {
   ...
   public static PathogenicityTriage  createNoDataTriageObject()
    {
	PathogenicityTriage pt = new PathogenicityTriage(Constants.UNINITIALIZED_FLOAT,Constants.UNINITIALIZED_FLOAT,Constants.UNINITIALIZED_FLOAT);	 
	return pt;
    }

    /**
     * This method creates a PathogenicityTriage object for a variant that we have
     * judged to be non-pathogenic.
     */
    public static PathogenicityTriage createNonPathogenicTriageObject(VariantType type)
    {
	PathogenicityTriage pt = new PathogenicityTriage(type,NON_PATHOGENIC);
	return pt;
    }

     /**
     * This method creates a PathogenicityTriage object for a variant that we have
     * found to belong to one of the classes of potentially pathogenic variants such 
     * as NONSENSE, FRAMESHIFT, etc.
     */
    public static PathogenicityTriage createPathogenicTriageObjectByType(VariantType type,float score)
    {
	PathogenicityTriage pt = new PathogenicityTriage(type,score);
	return pt;
    }
