 ...
 * The FrequencyVariantScore is created by the FrequencyFilter, one for each tested
 variant. The FrequencyVariantScore object can be used to ask whether the variant
 ...
 */
public class FrequencyVariantScore implements VariantScore {
    ...
    /**
     * A threshold for the frequency filter. It needs to be set once by the
     * FrequencyFilter class before filtering individual variants. It is
     * initialized to 200% to let all variants pass if for some reason the
     * client class does not set a value for it.
     */
    private static float threshold = 200f;

    /**
     * A threshold for frequency of a variant is used to filter out variants
     * with a higher population frequency.
     *
     * @param maxFreq Threshold, in percent
     */
    public static void setMaxFreqThreshold(float maxFreq) throws ExomizerInitializationException {
        if (maxFreq < 0f || maxFreq > 100f) {
            throw new ExomizerInitializationException("Illegal value for frequency threshold:" + maxFreq);
        }
        FrequencyVariantScore.threshold = maxFreq;
    }
    ...
    public boolean representedInDatabase() {
        if (this.dbSNPid != Constants.UNINITIALIZED_INT) {
            return true;
        }
        if (this.dbSNPmaf > Constants.UNINITIALIZED_FLOAT) {
            return true;
        } else if (this.espAllmaf > Constants.UNINITIALIZED_FLOAT) {
            return true;
        } else {
            return false;
        }
    ...
    public boolean hasFrequencyDataFrom_dbSNP() {
        return this.dbSNPmaf > Constants.UNINITIALIZED_FLOAT;
    }

    /**
     * @return true if this variant has frequency data from the exome variant
     * server (ESP)
     */
    public boolean hasFrequencyDataFromESP() {
        return this.espAllmaf > Constants.UNINITIALIZED_FLOAT;
    }
    ...
    public boolean has_dbSNPrsID() {
        return this.dbSNPid > Constants.UNINITIALIZED_INT;
    }
     ...
     */
    public FrequencyVariantScore(int dbsnp, float dbsnpmaf, float EAmaf, float AAmaf, float Allmaf) {
        this.dbSNPid = dbsnp;
     ...
     */
    public static FrequencyVariantScore createNoDataTriageObject() {
        FrequencyVariantScore ft = new FrequencyVariantScore(Constants.UNINITIALIZED_INT, Constants.UNINITIALIZED_FLOAT, Constants.UNINITIALIZED_FLOAT,
    ...
    public boolean passesFilter() {
        if (dbSNPmaf > FrequencyVariantScore.threshold) {
            return false;
        } else if (espAllmaf > FrequencyVariantScore.threshold) {
            return false;
        } else if (espEAmaf > FrequencyVariantScore.threshold) {
            return false;
        } else if (espAAmaf > FrequencyVariantScore.threshold) {
            return false;
        } else {
            return true;
        }
    ...
    public float filterResult() {
        /*
         ...
         if (max <= 0) return 1f;
         else if (max > FrequencyVariantScore.threshold) return 0f;
