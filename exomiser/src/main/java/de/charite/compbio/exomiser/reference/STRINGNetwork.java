...
public class STRINGNetwork implements Network {
    ...
    /** This hashmap contains as Keys Entrez Gene ids for genes; The corresponding values are lists
     * 	of String representing interaction paths that end at the gene and start at one of the seed genes.
     * The paths can be one or two interactions in length (first- or second-degree interactions).
     */    
    private Map<Integer,List<String>> interactionPath = null;

    /**
     * A set of the entrez gene ids of the seed genes. We use this to avoid entering paths
     * that cycle back to one of the seed genes from the first level paths (it is OK
     * if a candidate gene is also a seed gene, then we show these paths).
     */
    private Set<Integer> seedGeneIdSet=null;

    /**
     * @param c A connection to the postgreSQL Exomiser database.
     * @param seedGeneList A comma-separated list of Entrez Gene ids representing the seed genes
     * chosen by the user (e.g., all genes belonging to a phenotypic family).
     */
    ...
    /**
     * This method searches in the table entrez2sym for the Gene Symbol corresponding to
     * a given entrez gene id. For instance, the id  2200 corresponds to the symbol FBN1.
     * @param entrezID an NCBI Entrez Gene ID for a gene
     * @return The gene symbol corresponding to the entrez gene or null if no symbol can be found
     */
    ...
    /**
     * Returns an array list with all first and second degree paths to
     * the gene represented by entrezID. If there are no interactions,
     * it returns an empty arraylist.
     * @param  entrezID an NCBI Entrez Gene ID for a gene (one of the candidate genes from the exome)
     * @return List of first- and second degree interaction paths connection a seed gene to the candidate gene.
     */
     ...
     * {@link #interactionPath}. The key is the gene at the end
     * of the chain, i.e., on the other side of the path from 
     * a seed gene.
    ...
    /**
     * This method is called after a length-one path has been found from a
     * gene that is connected to a seed gene. The method adds all direct interaction
     * partners, so that in the end we have paths of length two from the seed genes.
     * For example, say we have a seed gene ABC1. Then we might use 
     * {@link #addSinglePath} to creat ethe path <code>ABC1 <-> DEF2</code>. This method
     * will extend that path to <code>ABC1 <-> DEF2 <-> GHI3</code>.
     * @param neighborID Entrez Gene id of a gene that is neighbor to a seed gene.
     * @param seedID the seed gene that neighborIDinteracts with
     * @param pathToDate String representation of the degree-one path coming from seedID, e.g., ABC1 <-> DEF2.
     */
    ...
    /**
     * This method is called to add interaction paths from the seed genes themselves.
	...
	try {
		...
		addDoublePath(id, seedID, path); /* id is the Entrez Gene id of the gene that interacts
					    with the seed gene (seedID). */
	    } 
	} catch (SQLException e) {
	    System.err.println("[WARN] SQLException" + e.getMessage());
	}
    ...
    /**
     * This method takes a comma separated list of entrez gene ids
     * representing the seed gene (e.g., all genes from some
     * OMIM Phenotypic Series), and finds all paths of lengths
     * one and two that emanate from them.
