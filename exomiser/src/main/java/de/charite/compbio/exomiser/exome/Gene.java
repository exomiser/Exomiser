 ...
 * the class {@link jannovar.reference.TranscriptModel TranscriptModel} stores information
 ...
 * {@link jannovar.reference.TranscriptModel TranscriptModel} object to annotate variants found
...
public class Gene implements Comparable<Gene>  {
     ...
     * object for the current VCF file. This object allows us to do 
     * segregation analysis for the variants associated with this gene, i.e., 
     * to determine if they are compatible with autosomal recessive,
     * autosomal dominant, or X-linked recessive inheritance.
     */
    private static Pedigree pedigree=null;

    /**
     * This method sets the pedigree for all Gene objects (it is a static method).
     * It is intended that pedigree filtering algorithms can use the genotypes associated
     * with this Gene and its Variants as well as the pedigree contained in the static
     * Pedigree object in order to perform inheritance filtering.
     ...
     * assumption that such genes are unlikely to be be true disease
     * genes, rather, by chance say 2 of 20 variants are score as highly
     * pathogenic by polyphen, leading to a false positive call.
     * This method downweights the {@link #filterScore} of this gene, which is
     * the aggregate score for the variants.
     * @param  threshold Downweighting occurs for variants that have this number or more variants.
     */ 
    public void downWeightGeneWithManyVariants(int  threshold) {
	if (this.variant_list.size() < threshold) return;
	...
	if (downweight>1f) downweight=1f;
     ...
     * <P>
     * Note that we <b>assume that genes have been filtered for
     * mode of inheritance before this function is called. This means
     * that we do not need to apply separate filtering for mode of
     * inheritance here</b>. The only thing we need to watch out for
     * is whether a variant is homozygous or not (for autosomal recessive
     * inheritance, these variants get counted twice).
	...
	if (mode == ModeOfInheritance.AUTOSOMAL_RECESSIVE) {
		...
		GenotypeCall gc = ve.getVariant().getGenotype();
		    ...
		    vals.add(x); /* Add the value a second time, it is homozygous */
		}
	    }
	} else { /* not autosomal recessive */
	    for (VariantEvaluation ve : this.variant_list) {
		float x = ve.getFilterScore();
		vals.add(x);
	    }
     ...
     */
    public List<VariantEvaluation> get_variant_list() { return this.variant_list; }
