...
a `GeneScore` which features filtered variants located in that gene compatible with a given mode of inheritance (MOI).

Each contributing variant for the `GeneScore` is then classified by the automated ACMG module
(see :ref:`acmg_assignment`) which will provide final ACMG classification e.g. Pathogenic / Likely Pathogenic / VUS /
Likely Benign / Benign and the ACMG posterior probability of pathogenicity which is combined with the phenotype and gene
variant score using a logistic regression trained on the `UK 100K genomes <https://www.genomicsengland.co.uk/initiatives/100000-genomes-project>`_
solved cases to produce the final Exomiser combined score.

Exomiser ranks these GeneScores according to descending combined score. The results are then written
...
HTML
====
The HTML output is the most human accessible output format which is designed to offer a ready overview of the results

.. figure:: images/exomiser-new-html-1-overview.png

The gene scores are shown according to the mode of inheritance under which they were considered and the Exomiser and
ClinVar ACMG classifications assigned to the variants contributing to the score. Clicking on the gene row will expand it
to reveal tabs which display the gene scores for each mode of inheritance, the known diseases associated with that gene
and the top phenotype matches.


Here we can see the top phenotype matches which Exomiser made to the FGFR2 disease, mouse, fish and PPI models:

.. figure:: images/exomiser-new-html-2-fgfr2-top-phenotype-matches.png


Switching to the 'Associated diseases' tab shows what disease information Exomiser has for the gene and the mode of
inheritance of the disease:

.. figure:: images/exomiser-new-html-3-fgfr2-associated-diseases.png


The 'Gene Score' tab will show summary information about the candidate variants which Exomiser has considered for this
gene which fit the mode of inheritance shown in the tab. Here there is only one heterozygous variant which fits the
autosomal dominant (AD) inheritance mode., under an autosomal recessive (AR) model either two heterozygous variants will
be displayed (AR compound heterozygous) or a single homozygous variant. There may also be a 'Non-contributing variants'
tab which will contain other variants which survived filtering but were not included in the score calculation.

.. figure:: images/exomiser-new-html-4-fgfr2-ad-gene-score.png


Finally, clicking on the variant overview will reveal details about all the variant pathogenicity scores and frequency
data Exomiser used in selecting the variant and also information from ClinVar about this variant (if present) and the
ACMG criteria automatically assigned by Exomiser along with the final ACMG classification. This process is described
here: :ref:`acmg_assignment`

.. figure:: images/exomiser-new-html-5-fgfr2-ad-gene-score-variant-details.png
...
specification. The JSON file is returned according to `JSON Lines <https://jsonlines.org>`_ format.
...
The JSON file can be relatively large, possibly even a few hundred MB, which can become a problem when there are
thousands of results. To this end, we have introduced the Parquet output file as a new default output (along with JSON
and HTML). It can be thought of as the best of the JSON, TSV_GENES and TSV_VARIANTS file merged together in a compact
file with strong typing and easy integration with data pipelines. The first few columns are displayed here as an example,
but the full columns can be shown using DuckDB and the SQL ``"DESCRIBE SELECT * FROM 'Pfeiffer-exomiser.parquet';"``

.. code-block::

     rank      variant                    geneSymbol       hgncId         ensemblGeneId      ncbiGeneId  moi       pValue    geneCombinedScore  genePhenotypeScore  geneVariantScore  variantScore  isContributingVariant  isWhiteListVariant  faile
   ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
     1         "10-123256215-T-G_AD"      "FGFR2"          "HGNC:3689"    "ENSG00000066468"  "2263"      "AD"      0.000002  0.97066            0.87752             1.0               1.0           true                   true                []
     2         "6-132203615-G-A_AD"       "ENPP1"          "HGNC:3356"    "ENSG00000197594"  "5167"      "AD"      0.000162  0.711517           0.62356             1.0               1.0           true                   true                []
     3         "7-76069617-TG-T_AD"       "ZP3"            "HGNC:13189"   "ENSG00000188372"  "7784"      "AD"      0.001186  0.400489           0.502852            0.995171          0.995171      true                   false               []
     4         "13-110855918-C-G_AD"      "COL4A1"         "HGNC:2202"    "ENSG00000187498"  "1282"      "AD"      0.049614  0.026842           0.506021            0.985988          0.985988      true                   false               []


As with the TSV_VARIANT  files the variant marked as 'true' in the ``isContributingVariant`` column were the ones which
were used to calculate the ``geneVariantScore`` and the ``geneCombinedScore``.

Another distinguishing feature of the parquet output is the some fields may hold lists or structs to describe their data,
e.g. the ``acmgEvidence`` column or the ``frequencies`` column.
