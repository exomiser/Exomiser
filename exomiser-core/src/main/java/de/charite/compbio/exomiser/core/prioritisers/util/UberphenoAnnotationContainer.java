...
public class UberphenoAnnotationContainer {
	 ...
	 * 
	 */
	public final Map<Integer, HashSet<UberphenoAnnotation>> container = new HashMap<Integer, HashSet<UberphenoAnnotation>>();
	...
	private Set<Term> getAnnotationsFiltered(int entrezId, Set<Integer> omimIdsToHandle, boolean isToExclude){
		...
		Set<Term> annots = new HashSet<Term>();
	...
	/**
	 * @param entrez
	 * @return a Gene symbol corresponding to the entrez ID or null if entrez id cannot be found.
	 */
	 ...
	 * @param uberphenoSlim
	 */
	public HashMap<Term, Double> calculateInformationContentUberpheno( Ontology uberpheno, SlimDirectedGraphView<Term> uberphenoSlim){

		Map<Term, Set<Integer>> term2geneids = new HashMap<Term, Set<Integer>>();
			...
			for (Term annotated : annotations){
				...
				for (Term annotatedAndInduced : uberphenoSlim.getAncestors(annotated)){
					...
					if (term2geneids.containsKey(annotatedAndInduced))
						...
						genesForTerm = new HashSet<Integer>();
	...
	private HashMap<Term, Double> calculateTermIC(Map<Term, Set<Integer>> term2objectIds, Ontology ontology) {
		...
		System.out.println("WARN: Frequency of "+frequencyZeroCounter+" terms was zero!! Calculated by -1 * (Math.log(1/(double)maxFreq)) = -1 * (Math.log(1/(double)"+maxFreq+")))");
