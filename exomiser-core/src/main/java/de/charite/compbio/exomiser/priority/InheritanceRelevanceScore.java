...
/**
 * This filter checks whether the variants are present in a fashion that is
 * compatible with autosomal dominant, recessive or X chromosomal.
 * <P>
 * Note: This class was renamed from InheritanceTriage on 11. February 2013
 *
 * @deprecated replaced by {@see de.charite.compbio.exomiser.filter.InheritanceFilterScore}
 * @author Peter N Robinson
 * @version 0.04 (April 6, 2013).
 */
@Deprecated
public class InheritanceRelevanceScore implements PriorityScore {

    /**
     * This is set to true of the variant, possibly together with other variants
     * of this gene, matches the inheritance pattern (AR,AD,X). Note that
     * because of the difficulty in calling hemizygous vs. homozygous calls on
     * the X chromosome, we do not distinguish between X recessive/dominant
     * inheritance.
     */
    ...
    public boolean passesFilter() {
        return matches_inheritance_pattern;
    }
    ...
    public float getScore() {
        if (matches_inheritance_pattern) {
            return 1f;
        } else {
            return 0f;
        }
