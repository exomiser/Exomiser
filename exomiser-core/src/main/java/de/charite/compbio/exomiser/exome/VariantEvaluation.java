...
public class VariantEvaluation implements Comparable<VariantEvaluation> {
     ...
     */
    private Map<FilterType, VariantScore> variantScoreMap = null;

    private List<String> mutationRefList = null;

    public VariantEvaluation(Variant v) {
        this.var = v;
        this.variantScoreMap = new HashMap<FilterType, VariantScore>();
    }
     ...
     *
     * @linkVariantScorezer.filter.Triage Triage} object to this variant. Such
     * objects represent the results of evaluation of this variant and may be
     * used for filtering or prioritization. The Integer is a constant from
     * {@link exomizer.common.FilterType FilterType} that identifies the type of
     * {@link exomizer.filter.Triage Triage} object being added (e.g.,
     * pathogenicity, frequency, etc).
     */
    public void addFilterTriage(VariantScore t, FilterType type) {
        this.variantScoreMap.put(type, t);
    }
    ...
    /**
     * Return an integer representing the chromosome. 1-22 are obvious, chrX=23,
     * ChrY=24, ChrM=25.
     */
     ...
     * and relevance of the Variant) by using data fromVariantScore* the
     * {@link exomizer.filter.Triage Triage} objects associated with this
     * Variant.
     * <P>
    ...
    public float getFilterScore() {
        float score = 1f;
        for (FilterType i : this.variantScoreMap.keySet()) {
            VariantScore itria = this.variantScoreMap.get(i);
    ...
    /**
     * @return the map of VariantScore objects that represent the result of
     * filtering
     */
    public Map<FilterType, VariantScore> getVariantScoreMap() {
        return this.variantScoreMap;
    }
