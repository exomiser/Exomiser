...
/**
 * Filter variants according to the phenotypic similarity to a set of
 * user-supplied HPO terms. This method implements the BOQA algorithm as
 * described in Bauer S et al (2012) Bayesian ontology querying for accurate and
 * noise-tolerant semantic searches. Bioinformatics. 28(19):2502-8. (PubMed
 * PMID: 22843981).+
 * <P>
 * Note that the class connects to the Exomizer SQL database in order to
 * retrieve the OMIM phenotype IDs that correspond to a given gene. Note that
 * there may be multiple such phenotype IDs per gene, and we need to choose the
 * disease with the highest BOQA score to assign to a gene that is being
 * prioiritized.
 *
...
public class BoqaPriority implements Priority {
     ...
     */
    private List<String> hpoList = null;
    /**
     * Map of results of BOQA analysis. Key: Name of disease (SHOULD BE OMIM ID)
     * and Value: BOQA posterior probability.
     */
    ...
    /**
     * A prepared SQL statement for finding the Phenotype IDs that match to a
     * gene.
     */
     ...
     * @param hpoAnnotationFile The annotation file obtained from
     * {@code http://purl.obolibrary.org/obo/hp/uberpheno/}
     * @param hpoTermList alist of HPO terms separated by comma, e.g.,
     ...
     * association file in order to get the unique ID of the disease. This
     * corresponds to a String such as " #101000 NEUROFIBROMATOSIS, TYPE II;
     * NF2;;NEUROFIBROMATOSIS, CENTRAL TYPE;; ACOUSTIC SCHWANNOMAS,
     * BILATERAL;;BILATERAL ACOUSTIC NEUROFIBROMATOSIS; BANF;; ACOUSTIC
     * NEURINOMA, BILATERAL; ACN". This method retrieves the corresponding MIM
     * ID, which is a 6-digit integer (here: 101000), which we get in form of a
     * ByteSrtring.
     */
    ...
    /**
     * This method performs the BOQA analysis for the HPO terms that have been
     * entered in the class variable {@link #hpoList}.
     */
     ...
     * "HP:0000407,HP:0009830,HP:0002858".
     */
    private void initializeHPOTermList(String hpoTermList) {
        String A[] = hpoTermList.split(",");
        if (A.length < 1) {
            logger.error("Could not parse any HPO terms from the input string \"{}\"", hpoTermList);
        }
        ...
        for (String a : A) {
            ...
            if (!a.startsWith("HP:") || a.length() != 10) { /* A well formed HPO term starts with "HP:" and has ten characters. */
                ...
                logger.error("Malformed HPO input string \"{}\". Could not parse term \"{}\"", hpoTermList, a);
            }
            hpoList.add(a);
        }
    }

    private void inputHPO(String hpopath, String assocpath) {
    ...
    /**
     * An implementation of Kahan's compensated summation which tries to
     * minimize precision errors when summing up ffp values.
     *
    ...
    class KahanSummation {
        ...
        /**
         * Add the given float to the accu/sum. Return the sum after the float
         * has been added.
         *
         * @param toAdd
    ...
    /**
     * This function uses the SQL table "omim" to look up the phenotype MIM
     * number(s) for a gene. The MIM numbers represent all of the diseases that
     * are associated with the gene. The MIM numbers should be identical with
     * the MIM numbers that were used to store the results of BOQA calculation
    ...
    private void setUpSQLPreparedStatement() {
        ...
        try {
            ...
            logger.error("Problem setting up SQL query:", query, e);
        }
    }
