...
/**
 * Prioritize genes according to the mode of inheritance. Essentially, we pass
 * the query on to the {@link exomizer.exome.Gene Gene} objects, that in turn
 * analyze the {@link jannovar.genotype.GenotypeCall GenotypeCall} objects of
 * each {@link jannovar.exome.Variant Variant} to determine if the distribution
 * of variants is compatible with a given
 * {@link jannovar.common.ModeOfInheritance ModeOfInheritance}.
 * <P>
 * This is implemented as a knockout. That is, if the variants are not
 * compatible with the mode of inheritance, the genes are simply removed.
 *
 * @deprecated replaced by {@see de.charite.compbio.exomiser.filter.InheritanceFilter}.
...
public class InheritancePriority implements Priority {
    ...
    /**
     * @param s A String (one of "AR","AD","X") representing the mode of
     * inheritance.
     * @return The corresponding constant or UNITIALIZED if s could not be
     * parsed.
     */
    ...
    public PriorityType getPriorityType() {
        return PriorityType.INHERITANCE_MODE_PRIORITY;
    }
    ...
    /**
     * This function filters the list of genes as to whether they contain one or
     * more variants that are compatible with the indicated mode of inheritance.
     * The actual work is done in the {@link exomizer.exome.Gene Gene} class.
     *
     * @param gene_list list of genes to be filtered.
     * <P>
     * Note that if a Gene contains at least one variant that is compatible with
     * the indicated mode of inheritance, it is retained, together with all of
     * the variants it has (users may want to see all of the variants present in
     * some gene). If a Gene contains no variant that is compatible with the
     * indicated inheritance pattern, it is completely removed.
     */
    @Override
