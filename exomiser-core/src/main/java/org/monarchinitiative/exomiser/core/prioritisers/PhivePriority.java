...
public class PhivePriority implements Prioritiser<PhivePriorityResult> {
    ...
    public Stream<PhivePriorityResult> prioritise(List<String> hpoIds, List<Gene> genes) {
        ...
        Set<Integer> wantedGeneIds = genes.stream().map(Gene::entrezGeneId).collect(ImmutableSet.toImmutableSet());
...
//                .filter(model -> model.getScore() > 0)
                .collect(groupingByConcurrent(GeneModelPhenotypeMatch::entrezGeneId, maxBy(comparingDouble(GeneModelPhenotypeMatch::score))));
    ...
    private Function<Gene, PhivePriorityResult> getPhivePriorityResult(Map<Integer, Optional<GeneModelPhenotypeMatch>> geneModelPhenotypeMatches) {
        return gene -> {
            return geneModelPhenotypeMatches.getOrDefault(gene.entrezGeneId(), Optional.empty())
                        ...
                        .orElse(newEmptyPhiveResultForGene(gene.entrezGeneId(), gene.geneSymbol()));
        };
    }

    private static PhivePriorityResult newEmptyPhiveResultForGene(int geneId, String geneSymbol) {
        GeneOrthologModel model = new GeneOrthologModel("", Organism.MOUSE, geneId, geneSymbol, "", "", List.of());
        return new PhivePriorityResult(geneId, geneSymbol, NO_MOUSE_MODEL_SCORE, new GeneModelPhenotypeMatch(NO_MOUSE_MODEL_SCORE, model, List.of()));
    }
