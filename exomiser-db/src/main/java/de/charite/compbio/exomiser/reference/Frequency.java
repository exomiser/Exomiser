 ...
 * to be used as an element of a {@code TreeSet} in the class 
 * {@link exomizer.io.dbSNP2FrequencyParser dbSNP2FrequencyParser}
...
public class Frequency implements Comparable<Frequency> {
    ...
    private int rsId;
    //the INFO filed from a VCF file
    ...
    public Frequency(byte chromosome, int position, String ref, String alt, int rsId, String info) {
     ...
     * {@link #dbSNPmaf}, {@link #espEA}, {@link #espAA}, and {@link #espAll}. This method
     * returns the maximum frequency amongst these four values.
     */
 ...
 and transform it into a String.
	...
	String s = String.format("%d|%d|%s|%s|%d|%f|%f|%f|%f%n",this.chromosome,this.pos,this.ref,this.alt,this.rsId,this.dbSNPmaf,this.espEA,
     ...
     * position as the current object. The method is intended to help avoid putting
	...
	if (! this.ref.equals(other.ref)) return false;
	if (! this.alt.equals(other.alt)) return false;
	return true;
    }


    /**
     * In a few cases, dbSNP contains multiple entries for some
     * given SNP. This method is used to reset an entry to the
     * values of some other entry in these cases (see the method
     * checkVariantForExomalLocationAndOutput from the class
     * dbSNP2FrequencyParser).
     */
